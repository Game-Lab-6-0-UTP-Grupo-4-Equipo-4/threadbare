...
@tool
class_name NatureMaxThrowingEnemy
...
func _spawn_projectile(target_label: String) -> NatureMaxProjectile:
	...
	var projectile: NatureMaxProjectile = scene.instantiate() as NatureMaxProjectile
...
func _on_got_hit(body: Node2D) -> void:
	if body is NatureMaxProjectile and not body.can_hit_enemy and not _is_defeated:
		return
	body.queue_free()
