...
class_name NatureMaxFillGameLogic
extends Node
...
func _ready() -> void:
	...
	if autostart or NatureMaxFillGameLogic.batalla_iniciada:
...
func _update_allowed_colors() -> void:
	...
	for enemy: NatureMaxThrowingEnemy in get_tree().get_nodes_in_group("throwing_enemy"):
